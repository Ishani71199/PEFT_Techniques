...
Implements training loop, validation loop and testing loop in Pytorch."""
...
class BaseModel:
    """Base class for pretrained frozen model"""
    def __init__(
        self,
    ...
    def get_parameter_count(self) -> float:
        """Returns trainable and total parameter count"""
    ...
    def __training_loop(self, train_loader, optimizer, description: str)->Tuple[float]:
        ...
        for batch in tqdm(train_loader, desc=description):
            ...
            loss = outputs.loss
            # Zero the gradients
            # Clear the previously accumulated gradients before doing backpropagation. 
            optimizer.zero_grad()
            # Backward_pass + step
            loss.backward()
            optimizer.step()
        ...
        # Batch --> Model --> Loss --> Gradients --> Weight Updates
        #                       |
        #            Track Loss and Accuracy

    def __validation_test_loop(self, val_test_loader, which: str = "Validation"
    ) -> Tuple[float]:
    ...
    def train(self, train_loader, val_loader, num_epochs: int = 10, learning_rate: float = 3e-4, es_patience: int = 5) -> Tuple[float]:
        ...
        scheduler = ReduceLROnPlateau(optimizer, patience=2) # If validation loss doesn’t improve for 2 epochs, reduce learning rate.
        ...
        for epoch in range(num_epochs):
            ...
            # Updates the learning rate based on validation loss. If val loss doesn’t improve for 2 epochs → LR will reduce.
            ...
            if val_loss < best_val_loss:
                ...
                if epochs_without_improvement >= es_patience:
                    print(
                        f"\nStopping early! Trained for {epoch + 1} / {num_epochs} epochs.\n"
                    )
                    break

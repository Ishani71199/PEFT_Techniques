"""Instead of updating all weights, it inserts trainable low-rank matrices (A and B) into the attention layers.
The original pre-trained weights are frozen.
This significantly reduces the number of trainable parameters while preserving performance."""

import torch
from .base_model import BaseModel
from functools import partial

# If in_dim = out_dim = 768, and rank = 4:
# Original Linear: 768×768 = 589,824 weights (frozen)
# LoRA: 768×4 + 4×768 = 6,144 weights (trainable)
...
class LoRA(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # At initialization, the LoRA component (A @ B) contributes nothing to the model output.
        # model behave exactly like the frozen pre-trained model at the start of fine-tuning.
        # 🧊 LoRA adds no effect initially → 🧠 Gradually learns a task-specific residual → 💡 Retains pre-trained knowledge, efficiently adapts
...
class LoRAModel(BaseModel):
    ...
    def __apply_lora(self) -> None:
        """
        Method that applies the LinearLoRA layer to certain Linear layers within the network
        Only adapt the linear layers in the Attention block, keep layers in the MLP frozen
        """
        print("Applying LoRA...")
